 <div class="content-wrapper">
     ...
     <section class="content">
         ...
         <div class="row">
             ...
             <div class="col-xs-12">
                 <form method="post" action="<?php echo base_url('user/jansunwai2'); ?>">
                     <div class="row">
                         <div class="col-md-2">
                             <div class="form-group">
                 ...
                 <div class="box">
                     <div class="box-header">
                         ...
                         <a href="<?php echo base_url(); ?>jansunwai/upload_csv" class="btn btn-info"
                             style="float: right;margin-right: 12px;">Upload jansunwai</a>
...
if (!empty($userRecords)) {
    ...
    foreach ($userRecords as $key => $record) {
        ...
        $updatedAt = isset($record->updatedAt) ? new DateTime($record->updatedAt) : null;
        $currentTime = new DateTime();

        // Calculate the time difference
        $timeDifference = $currentTime->diff($createdAt);
        $hoursDifference = ($timeDifference->days * 24) + $timeDifference->h;

        // Determine if the time is within the desired range
        $isEditable = $hoursDifference > 48 && $hoursDifference < 98;
        $expireTimestamp = $createdAt->getTimestamp() * 1000; // Default to createdAt for timer

        // If work status is Complete, calculate time difference between createdAt and updatedAt
        if ($record->work_status == "Complete" && $updatedAt) {
            $timeDifference = $updatedAt->diff($createdAt);
            $timeDiffText = $timeDifference->format("%dd, %hh, %im, %ss");
        }
?>
                                 <tr class="clickable-row" data-id="<?php echo $record->id; ?>" data-stage="2" style="cursor: pointer;">
                                     <td><?php echo $i++; ?></td>
                                 ...
                                 ?>
                                    ...
                                    $record->uploaded_file; ?>" title="Image" target="_blank">View File</a>
                                     ...
                                     <td class="text-center">
                                         ...
                                         <a class="btn btn-sm btn-success <?php echo $isEditable ? '' : 'disabled'; ?>"
                                             href="<?php echo base_url().'user/submit_form/'.$record->id; ?>/2"
                                             data-userid="<?php echo $record->id; ?>" title="Edit">
