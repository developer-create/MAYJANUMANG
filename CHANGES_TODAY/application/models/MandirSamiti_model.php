...
class MandirSamiti_model extends CI_Model {
    ...
    public function get_groups($search = null, $filters = array()) {
        ...
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like('mandir_samiti_groups.unique_id', $search);
            $this->db->or_like('booth.name', $search);
            $this->db->or_like('mandir_samiti_groups.gram_panchayat', $search);
            $this->db->or_like('mandir_samiti_groups.village', $search);
            $this->db->group_end();
        }
        
        if (!empty($filters['block'])) {
            $this->db->where('mandir_samiti_groups.block', (int)$filters['block']);
        }
    ...
    // Get distinct years from mandir_samiti_groups
    ...
    // Get distinct months from mandir_samiti_groups
    ...
    // Get distinct days from mandir_samiti_groups
    ...
    // Get blocks that have data in mandir_samiti_groups
    public function get_blocks_with_data() {
        $this->db->select('DISTINCT(b.id), b.name');
        $this->db->from('mandir_samiti_groups as g');
    ...
    public function get_total_members_count($filters = array()) {
        $this->db->select('SUM((SELECT COUNT(*) FROM mandir_samiti WHERE mandir_samiti.group_id = mandir_samiti_groups.id)) as total_members');
        $this->db->from('mandir_samiti_groups');
        $this->db->join('block', 'block.id = mandir_samiti_groups.block', 'left');
