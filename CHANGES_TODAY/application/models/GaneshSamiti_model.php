...
class GaneshSamiti_model extends CI_Model
{
    ...
    public function get_all_groups() {
        // Include booth name/number so views don't show raw booth id
        $this->db->select('ganesh_samiti_groups.*, block.name as block_name, booth.name as booth_name, booth.bnumber as booth_no,
                          (SELECT COUNT(*) FROM ganesh_samiti WHERE ganesh_samiti.group_id = ganesh_samiti_groups.id) as total_members');
        $this->db->from('ganesh_samiti_groups');
        $this->db->join('block', 'block.id = ganesh_samiti_groups.block', 'left');
        $this->db->join('booth', 'booth.id = ganesh_samiti_groups.booth_name', 'left');
    ...
    public function get_groups($search = null, $filters = array()) {
        ...
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like('ganesh_samiti_groups.unique_id', $search);
            $this->db->or_like('booth.name', $search);
            $this->db->or_like('ganesh_samiti_groups.gram_panchayat', $search);
            $this->db->or_like('ganesh_samiti_groups.village', $search);
            $this->db->group_end();
        }
        
        if (!empty($filters['block'])) {
            $this->db->where('ganesh_samiti_groups.block', (int)$filters['block']);
        }
    ...
    public function get_group_by_id($id) {
        ...
        $this->db->select('ganesh_samiti_groups.*, block.name as block_name, 
                          ganesh_samiti_groups.booth_name as booth_id,
                          booth.name as booth_display_name, booth.bnumber as booth_no');
        $this->db->from('ganesh_samiti_groups');
        $this->db->join('block', 'block.id = ganesh_samiti_groups.block', 'left');
        $this->db->join('booth', 'booth.id = ganesh_samiti_groups.booth_name', 'left');
        $this->db->where('ganesh_samiti_groups.id', $id);
        $query = $this->db->get();
        return $query->row_array();
    }

    public function get_total_members_count($filters = array()) {
        $this->db->select('SUM((SELECT COUNT(*) FROM ganesh_samiti WHERE ganesh_samiti.group_id = ganesh_samiti_groups.id)) as total_members');
        $this->db->from('ganesh_samiti_groups');
        $this->db->join('block', 'block.id = ganesh_samiti_groups.block', 'left');
    ...
    public function create_group($data) {
        ...
        $this->db->insert('ganesh_samiti_groups', $data);
    ...
    public function update_group($id, $data) {
        ...
        $this->db->where('id', $id);
        return $this->db->update('ganesh_samiti_groups', $data);
    }
    ...
    public function delete_group($id) {
        ...
        $this->db->delete('ganesh_samiti');
        
        // Delete group
        $this->db->where('id', $id);
        return $this->db->delete('ganesh_samiti_groups');
    }
    ...
    public function get_member_by_id($id) {
        $query = $this->db->get_where('ganesh_samiti', array('id' => $id));
        return $query->row_array();
    }
    ...
    public function create_member($data) {
        $data['created_at'] = date('Y-m-d H:i:s');
        $this->db->insert('ganesh_samiti', $data);
        return $this->db->insert_id();
    }
    ...
    public function update_member($id, $data) {
        $data['updated_at'] = date('Y-m-d H:i:s');
        $this->db->where('id', $id);
        return $this->db->update('ganesh_samiti', $data);
    }

    // Delete member
    public function delete_member($id) {
        $this->db->where('id', $id);
        return $this->db->delete('ganesh_samiti');
    }
    ...
    // Get distinct years from ganesh_samiti_groups
    ...
    // Get distinct months from ganesh_samiti_groups
    ...
    // Get distinct days from ganesh_samiti_groups
    ...
    // Get blocks that have data in ganesh_samiti_groups
    public function get_blocks_with_data() {
        $this->db->select('DISTINCT(b.id), b.name');
        $this->db->from('ganesh_samiti_groups as g');
    ...
    function getGaneshSamitiInfo($ganeshSamitiId) {
        return $this->get_group_by_id($ganeshSamitiId);
    }
    ...
    function addNewGaneshSamiti($ganeshSamitiInfo) {
        return $this->create_group($ganeshSamitiInfo);
    }

    function editGaneshSamiti($ganeshSamitiInfo, $ganeshSamitiId) {
        return $this->update_group($ganeshSamitiId, $ganeshSamitiInfo);
    }

    function deleteGaneshSamiti($ganeshSamitiId, $ganeshSamitiInfo = null) {
        return $this->delete_group($ganeshSamitiId);
    }

    function getAllGaneshSamitiRecords() {
        return $this->get_all_groups();
    }

    function getUniqueGaneshSamitiNames() {
        $this->db->select('DISTINCT(ganesh_samiti_name) as name');
        $this->db->from('ganesh_samiti_groups');
        $this->db->where('ganesh_samiti_name IS NOT NULL');
        $this->db->where('ganesh_samiti_name !=', '');
        $this->db->order_by('ganesh_samiti_name', 'ASC');
        $query = $this->db->get();
        return $query->result();
    }
